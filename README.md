...
AWS Glue、Amazon Redshift、PostgreSQL、Azure Synapse、Google BigQuery、Snowflake のデータに対して、
Databricks Unity Catalog から Federation でクエリを実行します。
Snowflake は Query Federation（JDBC）と Catalog Federation（Iceberg via S3）の2パターンに対応。
...
│  ┌────────────────┐ ┌──────────────────┐   ┌──────────────┐               │
│  │lhf_catalog_glue│ │lhf_catalog_      │   │lhf_query_    │               │
│  │   (AWS Glue)   │ │snowflake_iceberg │   │  snowflake   │               │
│  │  S3 直接読取   │ │ S3 Iceberg 読取  │   │  (JDBC)      │               │
│  └───────┬────────┘ └────────┬─────────┘   └──────┬───────┘               │
│          │                   │              ┌──────────────┐               │
│          │                   │              │lhf_query_    │               │
│          │                   │              │redshift/     │               │
...
└──────────┼───────────────────┼─────────────────────┼───────────────────────┘
           │                   │                     │
  ┌────────▼────────┐  ┌──────▼───────┐   ┌─────────▼─────────┐
  │  AWS Glue       │  │  Snowflake   │   │ Redshift/Postgres/│
  │  (S3 Parquet/   │  │  (Iceberg    │   │ Synapse/BigQuery  │
  │   Delta/Iceberg)│  │   on S3)     │   │ (JDBC pushdown)   │
  └─────────────────┘  └──────────────┘   └───────────────────┘
```
...
| Snowflake | Query | O | O |
| Snowflake Iceberg | Catalog | O | - |

### Snowflake Federation の2パターン

| パターン | 仕組み | 特徴 |
|---------|--------|------|
| **Query Federation** | Databricks → JDBC → Snowflake compute | 通常テーブル (非Iceberg) を参照 |
| **Catalog Federation** | Databricks → Snowflake メタデータ取得 → S3 直接読取 | Iceberg テーブル、Snowflake compute 不要で高速 |

Snowflake Iceberg Catalog Federation は Enterprise Edition 以上が必要です（Storage Integration + External Volume を使用）。
Trial アカウントは [Snowflake UI](https://app.snowflake.com) の Admin > Account から Enterprise にアップグレード可能です。
...
#### Snowflake アカウント (Snowflake 使用時)

Snowflake Query Federation / Iceberg Catalog Federation を使用する場合、Snowflake アカウントが必要です。

| 項目 | 説明 |
|------|------|
| **アカウント作成** | [signup.snowflake.com](https://signup.snowflake.com) で Trial アカウントを作成 |
| **エディション** | Iceberg Catalog Federation には **Enterprise Edition 以上**が必要 |
| **エディション変更** | Snowflake UI → Admin → Account → 「Upgrade to Enterprise」 |
| **リージョン** | AWS US West 2 (Oregon) を推奨 (S3 バケットと同リージョン) |

デプロイ時に以下の情報を入力します:

| パラメータ | 例 | 説明 |
|-----------|-----|------|
| Account URL | `https://xxx-yyy.snowflakecomputing.com` | Snowflake UI の URL |
| User | `MYUSER` | ログインユーザー名 |
| Password | - | パスワード |
| Warehouse | `COMPUTE_WH` | 使用する Warehouse 名 (デフォルト: COMPUTE_WH) |

> **Note**: Iceberg Catalog Federation では、Snowflake が S3 上に Iceberg テーブルを作成し、
> Databricks が Snowflake のメタデータ経由で直接 S3 を読み取ります (Snowflake compute 不要)。
> Storage Integration と External Volume を使用するため Enterprise Edition が必須です。
...
2. **Demo Notebook** リンクでノートブックを開く
3. SQL Warehouse (Pro/Serverless) をアタッチ
4. "Run All" で実行

ノートブック (`federation_demo.sql`) はデプロイ時に Job として自動実行済みです。Workflows > Job 履歴から最新の実行を開けば、各セルの実行結果を確認できます。
...
- [x] **AWS Glue Catalog Federation**: S3 上の Parquet/Delta/Iceberg を Glue 経由で読み取り
- [x] **Amazon Redshift Query Federation**: JDBC pushdown
- [x] **PostgreSQL Query Federation**: AWS RDS / Azure Flexible Server
- [x] **Azure Synapse Query Federation**: Serverless SQL pool (VIEW with VALUES)
- [x] **Google BigQuery Query Federation**: SA key JSON 自動生成
- [x] **Snowflake Query Federation**: JDBC → Snowflake compute (equipment_specs, spare_parts_inventory)
- [x] **Snowflake Iceberg Catalog Federation**: CONNECTION_SNOWFLAKE + storage_root → S3 直接読取 (operational_metrics, safety_incidents)
- [x] **OOP リファクタリング**: 1776 行モノリス → deployer/ パッケージ (per-resource 編集可)
- [x] **ワンコマンドデプロイ**: deploy / redeploy / destroy
- [x] **Snowflake 自動クリーンアップ**: destroy 時に Iceberg テーブル・External Volume・Storage Integration を自動 DROP

## TODO

- [ ] **OneLake (Microsoft Fabric) 対応**: Catalog Federation で Fabric Lakehouse をソースとして追加 (下記参照)
...
コードは `feature/multi-cloud-refactor` ブランチに存在。

### ブロッカー

| # | 課題 | 詳細 | 対応案 |
|---|------|------|--------|
| 1 | **クロステナント認証** | sandbox テナントと field-eng テナントが別。SP を Fabric workspace に追加するには Admin 権限が必要だが Contributor のみ | (a) Admin に SP 追加を依頼 (b) 同一テナント内に workspace を用意 (c) 自前の Fabric workspace を作成 |
| 2 | **Fabric テーブル認識** | DFS API でアップロードしたファイルが「正体不明」。Load Table API は schema-enabled lakehouse で非対応 | Fabric Spark endpoint / Notebook から `CREATE TABLE` |
| 3 | **Fabric 容量制限** | Pro/Trial ライセンス必須。新規 Lakehouse 作成が容量上限で失敗 | Trial ライセンス取得、または既存 Lakehouse 再利用 |
